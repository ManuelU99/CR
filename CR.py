...
if df_filtered.empty:
    ...
    # Show reason from CSV if exists
    if (
        ...
        if not match.empty:
            reason_text = match["Reason"].values[-1]
            st.subheader("📝 Comment from Graph_Quality_Control_Check.csv")
            st.text_area("Reason:", value=reason_text, height=150, disabled=True)
        else:
            st.info("ℹ️ No reason found for this graph in the CSV.")
