...
if df_filtered.empty:
    ...
    if not feedback_df.empty and len(selected_familia) == 1 and len(selected_tipo) == 1 and len(selected_ciclo) == 1 and len(selected_soaking) == 1 and len(selected_groups) == 1:
        match = feedback_df[
    ...
    is_correct = st.radio("Is this graph correct?", ("Yes", "No"), index=0 if default_selection == "Yes" else 1)
    ...
    if is_correct == "No":
        ...
        if reason:
            ...
            updated = pd.concat([feedback_df, entry], ignore_index=True) if not feedback_df.empty else entry
            csv_buffer = io.StringIO()
            updated.to_csv(csv_buffer, index=False)
            csv_data = csv_buffer.getvalue().encode('utf-8')
            st.download_button(label="⬇️ Download Updated Feedback CSV", data=csv_data, file_name="Graph_Quality_Control_Check.csv", mime='text/csv')
